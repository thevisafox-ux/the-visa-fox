...
const MobileNavLink: React.FC<{ to: string; icon: string; label: string; setIsLoading?: (loading: boolean) => void }> = ({ to, icon, label, setIsLoading }) => {
  ...
  return (
    ...
    >
      ...
      <span className={`text-xl mb-1 transform transition-all duration-300 drop-shadow-sm relative z-10 ${
        isActive 
          ? 'group-hover:scale-110 group-hover:rotate-3' 
          : 'group-hover:scale-105 group-hover:rotate-2'
      }`}>{icon}</span>
      <span className={`text-[10px] font-semibold transition-transform duration-300 tracking-wide leading-tight ${
        isActive 
